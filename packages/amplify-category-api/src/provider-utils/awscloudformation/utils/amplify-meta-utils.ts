...
export const getImportedAuthUserPoolId = (context: $TSContext): string | undefined => {
  ...
  if (authCategory && Object.keys(authCategory).length === 1) {
    const authResource = authCategory[Object.keys(authCategory)[0]];

    if (authResource.service === authServiceName && authResource.serviceType === 'imported') {
